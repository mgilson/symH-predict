...
conn = sqlite3.connect('omni.db')
cursor = conn.cursor()
cursor.execute("""
    CREATE TABLE OmniData (
      datetime,
      Bx,
      By,
      Bz,
...
for filename in args.files:
    with open(filename) as fin:
        for line in fin:
            ...
            by = float(fields[15])
            bz = float(fields[16])
            magb = float(fields[19])
            magv = float(fields[21])
            vx = float(fields[22])
            vy = float(fields[23])
            vz = float(fields[24])
            nn = float(fields[25])
            temp = float(fields[26])
            dynp = float(fields[27])
            ...
            subs = ','.join('?' * len(vector))
            query = 'INSERT OR REPLACE INTO OmniData VALUES (%s);' % subs
